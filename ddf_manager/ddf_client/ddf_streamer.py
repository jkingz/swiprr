...
class Streamer:
    ...
    def __init__(self, rets_session, format_type="STANDARD-XML-Encoded"):
        try:
    ...
    def retrieve_master_list(self):
        ...
        try:
            count = "-1"
            master_list = self.rets_session.search(
                resource="Property",
            ...
            if not master_list:
                logger.error(
                    "Failed to retreive master_list, returned value:%s",
                    str(master_list),
                )
                return [], count
            ...
            if master_list["ReplyCode"] != "0":
                logger.error(
                    "Search Failed by RETS Server, Returned Code: %s, Message: %s",
                    master_list["ReplyCode"],
                    master_list["ReplyText"],
                )
                return [], count

            if len(master_list["Data"]) != int(master_list["Count"]):
                logger.warning(
                    "Master list Count is:%s while master list len is :%s",
                    int(master_list["Count"]),
                    len(master_list["Data"]),
                )

            logger.info(
                "Master List Retrieved with %s Records", int(master_list["Count"])
            )
    ...
    def retrieve_active_records(
        self, last_update, offset=0, limit=SESSION_LISTINGS_COUNT
    ):
        """
        ...
        try:
            ...
            new_listings = self.rets_session.search(
                resource="Property",
                resource_class="Property",
                dmql_query=last_updated_query,
                limit=limit,
                offset=offset,
                format_type=self.format,
            )
            if new_listings["ReplyCode"] == "20201":
                ...
                logger.error(
                    "RETS Server Failure, Returned Code: %s, Message: %s",
                    new_listings["ReplyCode"],
                    new_listings["ReplyText"],
                )
                return [], count
            ...
            if new_listings:
                ...
                logger.warning(
                    "No Active Listing found or Failed to retreive active records"
                )
                return [], count
    ...
    def retrieve_by_id(self, ids_list, search_class="Property"):
        ...
        try:
            ...
            if ids_list:
                ...
                new_listings = self.rets_session.search(
                    resource=search_class,
                    resource_class=search_class,
                    dmql_query=query,
                    limit=None,
                    format_type=self.format,
                )
            ...
            if not new_listings:
                logger.error(
                    "Retreive by ID search returned empty values from the Server or Library"
                )
                return [], count
            ...
            if new_listings["ReplyCode"] == "20201":
                ...
                return (
                    [],
                    count,
                )  # Returning count as -1 as this should not return empty list
            ...
            if new_listings and isinstance(new_listings["Data"], list):
                ...
                logger.error(
                    "Failed to retreive Listings, returned value:%s ",
                    str(new_listings["Data"]),
                )
                return [], count

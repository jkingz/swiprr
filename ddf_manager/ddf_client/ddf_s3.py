...
class S3Handler:
    ...
    def __init__(self, media_dir, rets_session, format_type="STANDARD-XML-Encoded"):
        try:
    ...
    def delete_dir(self, dir_path):
        ...
        try:
            found = False
            for obj in self.s3.Bucket(AWS_STORAGE_BUCKET_NAME).objects.filter(
                Prefix=dir_path
            ):
                found = True
                logger.debug(
                    "Deleting DIR:%s", self.s3.Object(AWS_STORAGE_BUCKET_NAME, obj.key)
                )
    ...
    def delete_file(self, file_path):
        ...
        try:
            ...
            for obj in self.s3.Bucket(AWS_STORAGE_BUCKET_NAME).objects.filter(
                ...
                found = True
                logger.info(
                    "Deleting File:%s", self.s3.Object(AWS_STORAGE_BUCKET_NAME, obj.key)
                )
                self.s3.Object(AWS_STORAGE_BUCKET_NAME, obj.key).delete()
            return found
        except Exception as e:
    ...
    def failed_downloads_append_all(self, photos_dict, listing_key, failed_downloads):
        ...
        try:
            ...
            logger.error(e)
            logger.error(
                "Error in appending Listing %s to Failed Downloads", listing_key
            )

    def save_photo(self, xml_response, listing_id, photo_id=1, agent=False):
        ...
        dict_xml = xmltodict.parse(xml_response)
        data = dict_xml["RETS"]["DATA"]
        columns = dict_xml["RETS"]["COLUMNS"].split("\t")

        try:
            if agent:
                # Assumes that all images is going to jpg, which is dangerous!
                photo_path = f"{self.media_dir}/{self.agents_dir}/{listing_id}/.jpg"
                splitted_row = data.split("\t")
                photo_attributes = self.build_dict(columns, splitted_row)
                image_url = photo_attributes.get("MediaUrl", "")
                self.s3.Bucket(AWS_STORAGE_BUCKET_NAME).put_object(
                    Key=photo_path, Body=requests.get(image_url).content
                )
            else:
                photo_dir = f"{self.media_dir}/{self.listings_path}/{listing_id}"
                ...
                if type(data) == list:
                    ...
                    for row in data:
                        ...
                        logger.info(
                            f"- Downloading photo number {progress} out of {max_count}..."
                        )
                        ...
                        # Assumes that all images is going to jpg, which is dangerous!
                        photo_path = (
                            f"{photo_dir}/{photo_attributes.get('Order', '1')}.jpg"
                        )
                    ...
                    image_url = photo_attributes.get("MediaUrl", "")
                    # Assumes that all images is going to jpg, which is dangerous!
                    photo_path = f"{photo_dir}/{photo_attributes.get('Order', '1')}.jpg"
                    self.s3.Bucket(AWS_STORAGE_BUCKET_NAME).put_object(
                        Key=photo_path, Body=requests.get(image_url).content
                    )
        except Exception as e:
    ...
    def get_agent_photo(self, agent_id):
        """Gets an agent photo from the MLS server as a xml_stream and saves it to S3 (using save_photo function).
        ...
        try:
            ...
            if photo_object:
                xml_stream = photo_object[0]["content"]
                if len(xml_stream) < 500:
                    logger.debug("Listing Agent Photo is less than 500 Bytes")
                else:
                    self.save_photo(xml_stream, agent_id, agent=True)
    ...
    def get_photo(self, photo_id, listing_key, small=False):
        """Gets an agent photo from the MLS server as a xml_stream and saves it S3 (using save_photo function).
        Returns true the photo was retrieved and  saved successfully"""
        try:
            photo_downloaded = False
            object_type = "LargePhoto" if not small else "Photo"
            photo_object = self.rets_session.get_object(
                resource="Property",
                object_type=object_type,
                content_ids=listing_key,
                object_ids=str(photo_id),
            )
            if photo_object:
                xml_stream = photo_object[0]["content"]
                if len(xml_stream) < 500:
                    logger.debug(
                        "Error in Photo:%s for listing:%s Content:%s",
                        photo_id,
                        listing_key,
                        xml_stream,
                    )
                else:
                    self.save_photo(xml_stream, listing_key, str(photo_id))
                    photo_downloaded = True
            else:
                raise Exception(
                    "Failed to download Photo:%s for Listing:%s", photo_id, listing_key
                )
            return photo_downloaded
        except Exception as e:
            logger.error(e)
    ...
    def append_failed_photo(self, photo_id, listing_key, failed_downloads):
        ...
        try:
            if (listing_key, photo_id) not in failed_downloads:
                ...
                logger.error(
                    "Failed to Download Listing %s Photo id %s was added to failed Photos Downloads",
                    listing_key,
                    photo_id,
                )
        except Exception as e:
    ...
    def get_photos(self, listing, failed_downloads):
        ...
        try:
            ...
            photos_object = self.rets_session.get_object(
                resource="Property",
            ...
            for photo in photos_object:
                xml_stream = photo["content"]
                if len(xml_stream) < 500:
                    logger.error(
                        "Error in Photo:%s for listing:%s", photo_id, listing_key
                    )
                    self.append_failed_photo(photo_id, listing_key, failed_downloads)
                    continue
                else:
                    photo_id = photo["object_id"]
                    self.save_photo(xml_stream, listing_key, photo_id)
            return True
        except Exception as e:
            logger.error(e)
    ...
    def compare_photo(self, listing_key, photo_dict, prev_time_stamp):
        try:
            if photo_dict["LastUpdated"] != prev_time_stamp:
                ...
                logger.debug(
                    "Photo %s for listing %s was updated, New TS: %s, OLD TS: %s",
                    photo_dict["SequenceId"],
                    listing_key,
                    photo_dict["LastUpdated"],
                    prev_time_stamp,
                )
                if not downloaded:
            ...
            logger.error(e)
            logger.error(
                "Failed to compare photo for Listing:%s Photo:%s",
                listing_key,
                photo_dict["SequenceId"],
            )
        ...
        try:
            ...
            if isinstance(photos_dict_list, list):
                for photo_dict in photos_dict_list:
                    if (
                        photo_dict["SequenceId"] in prev_photos_dict.keys()
                    ):  # not new photo (sequence id existed)
                        self.compare_photo(
                            listing_key,
                            photo_dict,
                            prev_photos_dict[photo_dict["SequenceId"]],
                        )
                ...
                self.compare_photo(
                    listing_key,
                    photos_dict_list,
                    prev_photos_dict[photos_dict_list["SequenceId"]],
                )
        except Exception as e:
            logger.debug(e)
    ...
    def download_photos(
        self, listings, previous_photos, progress=0, to_track_maximum_number=0
    ):
        ...
        try:
            ...
            for listing in listings:
                # Use this to have more feedback on shell

                logger.info(
                    f"Downloading photos for property with a DDF ID of {listing['ID']}..."
                )
                if progress and to_track_maximum_number:
                    logger.info(
                        f"Progress on property: {progress} out of {to_track_maximum_number}"
                    )
                    progress += 1
                ...
                try:
                    ...
                    if (
                        "Photo" in listing.keys()
                        and "PropertyPhoto" in listing["Photo"]
                    ):
                        ...
                        if listing_key in previous_photos.keys():
                            ...
                            logger.debug(
                                "Previous Photos found for listing:%s", listing_key
                            )
                        else:
                            logger.debug("New Listing :%s", listing_key)
                            if not self.get_photos(listing, failed_downloads):
                                self.failed_downloads_append_all(
                                    photos_dict, listing_key, failed_downloads
                                )
                    else:
                    ...
                    logger.error(e)
                    logger.error(
                        "Failed to download Photos for Listing:%s", listing_key
                    )
                    self.failed_downloads_append_all(
                        photos_dict, listing_key, failed_downloads
                    )
            ...
            if len(failed_downloads) > 0:
                ...
                logger.info(
                    "All New %s Listing Photos were Downloaded Successfully",
                    len(listings),
                )
            return True, failed_downloads, progress
        except Exception as e:
            logger.error(e)
        ...
        try:
            for listing_key in listings_keys:
                ...
                if not photos_deleted:
                    logger.error(
                        "Failed to delete photos for listing : %s", listing_key
                    )
        except Exception as e:
            logger.error(e)
            logger.error("Error in Deleting old Listings Photos")

    def build_dict(self, columns, batch_data):
        """
        Makes the unformatted xml list to a proper dictionary
        Easier to manage this way. (Makes things readable)
        """
        formatted_dict = {}
        index = 0
        while index < len(columns):
            formatted_dict = self.add_element_to_dict(
                formatted_dict, columns[index], batch_data[index]
            )
            index += 1
        return formatted_dict

    def add_element_to_dict(self, to_format_dict, column, data):
        """
        Adds element to a dictionary
        """
        to_format_dict[column] = data

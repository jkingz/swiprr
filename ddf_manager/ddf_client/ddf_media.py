...
class MediaHandler:
    ...
    def delete_dir(dir_path):
        ...
        try:
            if os.path.exists(dir_path):
                ...
                return True
            logger.error(
                "Photo DIR Doesn't exist Delete Command was ignored: %s", dir_path
            )
            return False
        except Exception as e:
            logger.error(e)
    ...
    def get_agent_photo(self, agent_id):
        """Gets an agent photo from the MLS server as a xml response and saves it locally (using save_photo function).
        ...
        try:
            ...
            if photo_object:
                xml_response = photo_object[0]["content"]
                if len(xml_response) < 500:
                    logger.debug("Listing Agent Photo is less than 500 Bytes")
                else:
                    self.save_photo(xml_response, agent_id, agent=True)
    ...
    def get_photo(self, photo_id, listing_key, small=False):
        """Gets a single listing photo from the MLS server as a xml response and saves it locally (using save_photo function).
        Returns true the photo was retrieved and  saved successfully"""
        ...
        try:
            ...
            if photo_object:
                ...
                if len(xml_response) < 500:
                    ...
                    self.save_photo(xml_response, listing_key, str(photo_id))
                    photo_downloaded = True
            else:
                raise Exception(
        ...
        try:
            ...
            for photo in photos_object:
                ...
                if len(xml_response) < 500:
                    ...
                    self.save_photo(xml_response, listing_key, photo_id)
            return True
        except Exception as e:
            logger.error(e)

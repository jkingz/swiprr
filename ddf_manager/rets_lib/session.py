...
from .parsers.get_object import MultipleObjectParser, SingleObjectParser
...
class Session(object):
    ...
    def __init__(
        ...
        metadata_format="STANDARD-XML-Encoded",
    ):
        """
        Session constructor
    ...
    def _make_metadata_request(self, meta_id, metadata_type=None):
        ...
        try:
            ...
            if self.metadata_format != "STANDARD-XML-Encoded" and e.reply_code in [
                "20513",
                "20514",
            ]:
                self.metadata_responses.pop(key, None)
                self.metadata_format = "STANDARD-XML-Encoded"
                return self._make_metadata_request(
    ...
    def search(
        ...
        parameters = {
            ...
            "Count": 1,
            "Format": format_type,
            #'format_type': 'COMPACT-DECODED',
            #'format_type': 'STANDARD-XML',
            #'StandardNames': 0,
        }
        ...
        # if the Select parameter given is an array, format_type it as it needs to be
        ...
        # print(parameters)
        try:
            if format_type == "STANDARD-XML-Encoded":
    ...
    def get_response_lookup_values(self, resource, lookup_name):
        """
        Get the raw response from lookup values
        :param meta_id: The name of the resource, class, or lookup to get metadata for
        :param metadata_type: The RETS metadata type
        :return: raw response object
        """
        return self._make_metadata_request_and_return_as_raw_response(
            meta_id=resource + ":" + lookup_name, metadata_type="METADATA-LOOKUP_TYPE"
        )

    def _make_metadata_request_and_return_as_raw_response(self, meta_id, metadata_type=None):
        """
        Get the Metadata. The Session initializes with 'COMPACT-DECODED' as the format type. If that returns a DTD error
        then we change to the 'STANDARD-XML' format and try again.
        :param meta_id: The name of the resource, class, or lookup to get metadata for
